...
describe('WomenLandingPage', () => {
    it('renders landing page content', async () => {
        getProducts.mockResolvedValue([
            { id: 1, name: 'Women Product', category: 'women', price: 10, images: [] }
        ]);
        render(<WomenLandingPage />);
        expect(screen.getByText(/Elegance/i)).toBeInTheDocument();
        await waitFor(() => {
            expect(screen.getByText('Women Product')).toBeInTheDocument();
        });
    });
});
